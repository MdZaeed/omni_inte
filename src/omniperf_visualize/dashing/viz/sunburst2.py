...
def sunburst(data_loader):
    ...
    for reg in regions:
        belief_map[reg] = {}
        ## TZI: 12/30/2020-- changed rsm_score to errors, otherwise the belief calculation does not make sense. Belief should be computed based on error, not rsm.
        for resource, res_percent_err in errors[reg].items():
            ...
            if use_belief:
                ...
                print("[BELIEF] %s : %s : %s" % (resource, res_percent_err, belief_map[reg][resource]))


    ## NEW: TZI: 12/16/2020 Normalizing across all regions -- 
    belief_min_across_all_regions = 1e6
    belief_max_across_all_regions = 1e-6
    for reg in regions:
        if(not belief_map[reg]):
            continue
        print ('<DEBUG> Finding overal min, max...........', reg, belief_map[reg], belief_min_across_all_regions, belief_max_across_all_regions)
        belief_min = min(belief_map[reg].values())
        belief_max = max(belief_map[reg].values())
        if (belief_min_across_all_regions > belief_min):
            belief_min_across_all_regions = belief_min
        if (belief_max_across_all_regions < belief_max):
            belief_max_across_all_regions = belief_max
    ...
    for reg in regions:
        ...
        for resource in belief_map[reg]:
            belief_map[reg][resource] = (belief_map[reg][resource] - belief_min_across_all_regions) / (belief_max_across_all_regions - belief_min_across_all_regions)

            if belief_map[reg][resource] < BELIEF_THRESHOLD:
                print("<DEBUG> L249: Removing %s from %s, %s" % (resource, reg, belief_map[reg][resource]))
                keys_to_remove.append(resource)
        how_many_keys_left = len(belief_map[reg])
        for key in keys_to_remove:
            del belief_map[reg][key]
            how_many_keys_left -= 1

        # REMOVE a region that does not have any resources left as important.
        # TZI: Dec 2020 new addition
        if how_many_keys_left <= 0:
            print("<DEBUG> L259: REMOVING REGION: ", reg)
            regions.remove(reg)
    ...
    for reg in regions:
        ...
        for event, percent_error in ev_percent_error[reg].items():
            ...
            if use_belief:
                belief_ev_map[reg][resource] = np.exp(-lam * percent_error)
    ...
    for reg in regions:
        ...
        for event, event_belief in belief_ev_map[reg].items():
            if event in data_loader.ev_to_res_map:
                print ("<DEBUG> L316: region, event: event_belief: ", reg, event, event_belief)
    ...
    ## NEW: TZI: 12/30/2020 Normalizing across all regions -- 
    belief_min_across_all_regions = 1e6
    belief_max_across_all_regions = 1e-6
    ...
    #           belief_res_ev_map[reg][resource][event] = (event_belief - belief_min_across_all_regions) / (belief_max_across_all_regions - belief_min_across_all_regions)
     ...
     #               csv_writer.writerow([app_name, reg, resource, event, belief_res_ev_map[reg][resource][event]])
        ...
        #for resource in resources_to_remove:
            #print("<DEBUG> L362: Deleting %s from %s" % (resource, reg))
    ...
    for reg in belief_res_ev_map:
        ...
        for resource in belief_res_ev_map[reg]:
            ...
            for event, belief in belief_res_ev_map[reg][resource].items():
                ...
                print('L378: ', belief, belief/belief_sum)
