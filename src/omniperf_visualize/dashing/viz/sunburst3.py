...
def shorten_kernel_names(kernel_name):
    # print(kernel_name)
    match = re.search(r'.*<.*>.*\(.*\).*\[.*\]$', kernel_name)
    if match:
        short_name = re.sub(r'<.*>.*\(.*\).*\[.*\]$', '', kernel_name)
        short_name = short_name.replace('void_','')
        return short_name
    else:
        return kernel_name
...
def conv_str(ev):
    ...
    #     shortened_event_name += ''
        ...
        shortened_event_name += 'WVS'
    else:
        shortened_event_name += ev
    return shortened_event_name

def shorten_event_name(event_name):    
    shortened_event_name = event_name
    ...
    if event_name == 'l2_subp0_write_sysmem_sector_queries':
        ...
        shortened_event_name = 'p0_w_hit'    # elif (ev == 'misses'):
    #     shortened_event_name += 'miss'
    # elif (ev == 'subp0'):
    #     shortened_event_name += 'p0'
    # elif (ev == 'subp1'):
    ...
    #     shortened_event_name += 'st'
    # elif (ev == 'store'):
    #     shortenedevent_str_list_event_name += 'st'
    # elif (ev == 'read'):
    #     shortened_event_name += 'r'
    # elif (ev == 'write'):
    #     shortened_event_name += 'w'
    ...
    elif event_name == 'TA_FLAT_WRITE_WAVEFRONTS_sum':
        shortened_event_name = 'TA_FLAT_WR_WVS_sum'
    elif event_name == 'TA_FLAT_READ_WAVEFRONTS_sum':
        shortened_event_name = 'TA_FLAT_RD_WVS_sum'
    elif event_name == 'TA_FLAT_READ_WAVEFRONTS_std':
        shortened_event_name = 'TA_FLAT_RD_WVS_std'
    elif event_name == 'TA_FLAT_WRITE_WAVEFRONTS_std':
        shortened_event_name = 'TA_FLAT_WR_WVS_std'
    elif event_name == 'TCP_TCP_TA_DATA_STALL_CYCLES_std':
        shortened_event_name = 'TCP_TA_DT_STL_CYL_std'
...
def sunburst(data_loader):
    ...
    # print('Zayed data: ', normed_belief_res_ev_map)
    ...
    for reg in regions:
        ...
        if normed_belief_map[reg]:
            ...
            ids.append(reg)
            pairs.append([reg])
            labels.append(shorten_kernel_names(reg))
