...
def create_barchart_2(data_loader):
    ...
    for i in range(len(recreate_barchart_2gions)):
        if ':' in regions[i]:
...
def compare_rsm_barcharts_per_block_both(data_loaders, global_options):
    ...
    for config in global_options['compare_kernels']:
        ...
        fig.update_xaxes(showline=True, linewidth=2, linecolor='black',tickangle=270)
        fig.update_yaxes(showline=True, linewidth=2, linecolor='black', gridcolor='black',zeroline=True,zerolinecolor='black')
        ...
        if save_figure:
            ...
            for cs in compat_pair_strings:
                ...
                dl2_name, reg2 = key_reg2.split(':', 1)
                confname += reg1
            file_path = '%s_%s.pdf' % (dl1_name, dl2_name)
            dir_path = os.path.join('viz_output', 'barchart')
            dir_path = os.path.join('viz_output', 'barchart')
