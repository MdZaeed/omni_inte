...
def output_groups_new(tcs, mis, column_label, output_filename, num_hidden_layers, thresh=0, prefix='',):
    #f = safe_open(prefix + '/text_files/groups.txt', 'w+')
    #h = safe_open(prefix + '/text_files/summary.txt', 'w+')
    f = safe_open('output/y_'+output_filename+'_'+str(num_hidden_layers)+'.txt', 'w+')
    h = safe_open('output/summaries/summary_'+output_filename+'_'+str(num_hidden_layers)+'.txt', 'w+')
    h.write('Group, TC\n')
    m, nv = len(tcs), len(mis[0])
    for j in range(m):
        f.write('$Y%d: %0.3f\n' % (j, tcs[j]))
        h.write('%d, %0.3f\n' % (j, tcs[j]))
        inds=[i for i in range(len(column_label))]
    ...
    f.close()
    h.close()
def output_groups(tcs, mis, column_label, num_hidden_layers, thresh=0, prefix='', ):
    #f = safe_open(prefix + '/text_files/groups.txt', 'w+')
    #h = safe_open(prefix + '/text_files/summary.txt', 'w+')
    f = safe_open('output/y_output/output_'+str(num_hidden_layers)+'.txt', 'w+')
    h = safe_open('output/summaries/summary_'+str(num_hidden_layers)+'.txt', 'w+')
    h.write('Group, TC\n')
    m, nv = len(tcs), len(mis[0])
    for j in range(m):
        f.write('Y%d: %0.3f\n' % (j, tcs[j]))
        h.write('%d, %0.3f\n' % (j, tcs[j]))
        inds=[i for i in range(49)]
        #inds = np.argsort(-mis[j])
        for ind in inds:
            f.write(column_label[ind] + ', %0.3f\n' % mis[j][ind])
    h.write('Total: %0.3f' % np.sum(tcs))
    f.close()
    h.close()
def output_groups_backup(tcs, mis, column_label, thresh=0, prefix=''):
    #f = safe_open(prefix + '/text_files/groups.txt', 'w+')
    #h = safe_open(prefix + '/text_files/summary.txt', 'w+')
    f = safe_open('output/groups.txt', 'w+')
    h = safe_open('output/summary.txt', 'w+')
    h.write('Group, TC\n')
    m, nv = len(tcs), len(mis[0])
    for j in range(m):
