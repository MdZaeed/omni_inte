...
class Sieve(object):
    ...
    def fit(self, x):#this function essentially derives the latent factors from X, does this by assigning weights (ws) based on the number of latent factors
        #learns from model to assign weights accordingly
        """Learn n_hidden latent factors that extract as much common information from X as possible."""
        x = np.asarray(x, dtype=float)
        # print('Zayed x',x.shape)
        ns, nv = x.shape
        ...
        # print('Zayed mean_x' , self.mean_x)
    ...
    def transform(self, x, remainder=False, level=-1): # assigns y based on weights and array of inputs
        #y_weights={}

        #make this print out y and x factors
        ...
        ys = []
        for j in range(self.n_hidden):
            #print ('x at this variable: ' , x)
            y = np.dot(self.ws[j], x.T) #tells correlation between 2 vectors (look at ws), calculate latent variable y, ws is the weights based on the amount of latent factors and the input x
            #print("random x: ", x[:,nv+j])
            """y_weights[tuple(y)]=self.ws[j].tolist()"""
            x[:, nv + j] = y
            for i in range(nv + j):
                x[:, i] -= self.moments[j]['X_i Y'][i] / (self.moments[j]["Y^2"] - self.noise ** 2) * x[:,
                                                                                                      nv + j]  # Don't add noise for transform, only in training
            ys.append(y) #append latent variable
            ...
            if j == np.mod(level, self.n_hidden):
                ...
                #understand transform, fit, and init function in this file, map to paper by wed
